...
async function getBookDetails(title, author) {
    const result = await axios.get(`${bookTitleSearchURL}${await addPLusSign(title)}&author=${await addPLusSign(author)}`)
    ...
    const response = await axios.get(`${bookDescriptionSearchURL}${descriptionKey}`)
...
app.post("/add_book", async (req,res) => {
    try {
        ...
        const {title: bookTitle, author: author_name, description: bookDescription, bookCover: bookCoverImage} = await getBookDetails(title, author)
        try {
            await db.query(
                "INSERT INTO books (title, author, review, rating, date, description, book_cover) VALUES ($1, $2, $3, $4, $5, $6, $7)",
                [bookTitle, author_name, review, rating, date, bookDescription, bookCoverImage])   
        } catch (error) {
            console.log(error)
...
app.post("/update", async (req, res) => {
    ...
    const date = new Date().toDateString();
    rating = parseInt(rating);
    await db.query("UPDATE books SET title = $1, author = $2, rating = $3, review = $4, date = $5 WHERE id = $6",
